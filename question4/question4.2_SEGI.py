...
def simulate(network, timeSteps):
    ...
    for s in range(timeSteps):
        ...
        totalServers_SEGI[:, s] += network[SEGI]['servers']
...
def main():
    ...
    averageServers_SEGI = np.zeros((3, STEPS))

    for sim in range(SIMULATIONS):
        network = deepcopy(initialNetwork)
        totalServers_SEGI = simulate(network, STEPS)
        averageServers_SEGI += totalServers_SEGI
    
    averageServers_SEGI /= SIMULATIONS

    # Plotting
    plt.plot(range(STEPS), averageServers_SEGI[V], label='Vulnérables (V)')
    plt.plot(range(STEPS), averageServers_SEGI[I], label='Infectés (I)')
    plt.plot(range(STEPS), averageServers_SEGI[P], label='Protégés (P)')

    # Plot
    plt.xlabel('Temps')
    plt.ylabel('Nombre de serveurs')
    plt.title('Nombre moyen de serveurs "SEGI" : {} Simulations'.format(SIMULATIONS))
    plt.legend()
    plt.show()
